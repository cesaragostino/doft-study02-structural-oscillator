...
subset of $M=11$ representative materials spanning Type I, Binary, Iron-based, 
and Hydride families.
...
Figure X displays the relationship between the DOFT participation number $N$ 
and the experimental $\xi_0$. We observe a strong positive correlation ($R^2 \approx 0.90$) 
described by a power law $N \propto \xi_0^\alpha$, with $\alpha \approx 0.8$. 
This trend bridges three orders of magnitude in length scale:
\begin{itemize}
...
While the global trend is robust, specific families like iron-based superconductors 
(e.g., FeSe) show deviations from the universal fit, pointing to family-specific 
confinement geometries (e.g., 2D vs 3D) that merit further study. Nevertheless, 
this connection provides a first bridge between the thermodynamic frequency 
...
\begin{figure}
  ...
  significantly lower structural noise (Cliff's $\delta \approx -0.47$).
...
length $\xi_0$ for 11 representative superconductors. The dashed line shows 
