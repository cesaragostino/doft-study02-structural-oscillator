...
def main() -> None:
    ...
    gen_cmd = [
        "python3",
        str(REPO_ROOT / "src/tools/generate_doft_configs.py"),
        "--results-root",
    ...
    if not args.skip_participation:
        ...
        part_cmd = [
            "python3",
            str(REPO_ROOT / "src/tools/compute_integer_participation.py"),
            "--materials-csv",
            str(args.materials_csv),
            "--noise-csv",
            str(noise_csv),
        ...
        plot_cmd = [
            "python3",
            str(REPO_ROOT / "src/tools/plot_integer_participation.py"),
            "--participation-csv",
            str(part_summary),
            "--materials-csv",
            str(args.materials_csv),
        ...
        if args.experimental_csv.exists():
            ...
            val_cmd = [
                "python3",
                str(REPO_ROOT / "src/tools/validation_coherence.py"),
                "--participation-csv",
                str(part_summary),
                "--experimental-csv",
    ...
    if args.run_sensitivity and not args.skip_noise:
        if not noise_json.exists():
            ...
            for mat in materials:
                ...
                sens_cmd = [
                    "python3",
                    str(REPO_ROOT / "src/tools/sensitivity_analysis.py"),
                    "--materials-csv",
                    str(args.materials_csv),
                    "--noise-json",
                    str(noise_json),
    ...
    if args.run_loo and not args.skip_noise:
        if not noise_json.exists():
            ...
            loo_cmd = [
                "python3",
                str(REPO_ROOT / "src/tools/loo_validation.py"),
                "--noise-json",
                str(noise_json),
                "--output",
                str(loo_out),
